function [finalEVM,CRCFail] = PDSCHEVM(enb,cec,rxWaveform)

CRCFail = false; % Set flag
CRCFailures = 0;
sLen = 0;
    ...
    if (isfield(enb,'TMN') || any(enb.PDSCH.CodedTrBlkSizes(:,enb.NSubframe+1)~=0))
        ...
        for e = 1:2
            ...
            if (isfield(enb,'TMN'))
                ...
                if (sum(crc)~=0)
                    CRCFail = true;
                    CRCFailures = CRCFailures + 1;
                    fprintf(repmat('\b',1,sLen));
                    s = sprintf('CRC failed on decoded data. Subframe %d not taken for EVM. %d Failures.',enb.NSubframe,CRCFailures);
                    fprintf(s);
                    sLen = length(s);
                end
